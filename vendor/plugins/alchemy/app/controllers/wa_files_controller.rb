class WaFilesController < ApplicationController
  ...
  def index
    ...
    if params[:per_page] == 'all'
      @wa_files = WaFile.find(
        :all,
        :order => :name,
        :conditions => cond
      )
    else
      @wa_files = WaFile.paginate(
        :all,
        :order => :name,
        :conditions => cond,
        :page => (params[:page] || 1),
  ...
  def create
    @wa_file = WaFile.new(:uploaded_data => params[:Filedata])
  ...
  def archive_overlay
    ...
    @files = WaFile.all(:order => :name, :conditions => condition)
    render :layout => false
  end
