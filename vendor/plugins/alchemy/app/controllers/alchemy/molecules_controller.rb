class Alchemy::MoleculesController < ApplicationController
  ...
  def index
    ...
    @wa_molecules = Molecule.find_all_by_wa_page_id_and_public(@wa_page_id, true)
  end
  
  def new
    @wa_page = WaPage.find_by_id(params[:page_id])
    @molecule_before = Molecule.find_by_id(params[:molecule_before_id], :select => :id)
    @molecules = Molecule.all_for_layout(@wa_page, @wa_page.page_layout)
  end
  ...
  def show
    @wa_molecule = Molecule.find(params[:id])
