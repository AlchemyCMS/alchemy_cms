class Alchemy::MailsController < ApplicationController
  ...
  def create
    @wa_mail = Mail.new(params[:wa_mail])
    @wa_mail.ip = request.remote_ip
    molecule = Molecule.find_by_id(@wa_mail.contact_form_id)
    @page = molecule.page
    if @wa_mail.save
      if params[:mail_to].blank?
        mail_to = molecule.atom_by_name("mail_to").content
      else
        mail_to = Alchemy::Configuration.parameter(:mailer)[:mail_addresses].detect{ |c| c[0] == params[:mail_to] }[1]
      end
      mail_from = molecule.atom_by_name("mail_from").content
      subject = molecule.atom_by_name("subject").content
      Mailer.deliver_mail(
        @wa_mail,
        mail_to,
        mail_from,
      ...
      elsif !molecule.atom_by_name("success_page").blank?
        redirect_to show_page_url(molecule.atom_by_name("success_page").content)
      else
        redirect_to :controller => 'alchemy/pages', :action => 'show', :urlname => Page.language_root(session[:language]).urlname
      end
    else
