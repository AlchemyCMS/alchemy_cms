class Molecule < ActiveRecord::Base
  ...
  stampable
  has_many :atoms, :order => :position, :dependent => :destroy
  belongs_to :page
  ...
  # Returns next Molecule on self.page or nil. Pass a Molecule.name to get next of this kind.
  def next(name = nil)
    if name.nil?
  ...
  # Returns previous Molecule on self.page or nil. Pass a Molecule.name to get previous of this kind.
  def prev(name = nil)
    if name.nil?
      find_conditions = ["public = 1 AND page_id = ? AND position < ?", self.page.id, self.position]
    else
  ...
  def atom_by_name(name)
    self.atoms.find_by_name(name)
  end

  def atom_by_type(atom_type)
    self.atoms.find_by_atom_type(atom_type)
  end

  def all_atoms_by_name(name)
    self.atoms.find_all_by_name(name)
  end

  def all_atoms_by_type(atom_type)
    self.atoms.find_all_by_atom_type(atom_type)
  end
  
  # creates a new molecule for page as described in /config/alchemy/molecules.yml from molecule_name
  def self.create_from_scratch(page_id, molecule_name)
    ...
    molecule_scratch.delete("atoms")
    molecule_scratch.delete("available_atoms")
    molecule = Molecule.new(
      molecule_scratch.merge({:page_id => page_id})
    )
    molecule.save!
    molecule
  end
  
  # pastes a molecule from the clipboard in the session to page
  def self.paste_from_clipboard(page_id, molecule, method, position)
    copy = self.copy(molecule, :page_id => page_id)
  ...
  def preview_text
    text = ""
    begin
      my_atoms = my_description["atoms"]
      unless my_atoms.blank?
        atom_flagged_as_preview = my_atoms.select{ |a| a["take_me_for_preview"] }.first
        if atom_flagged_as_preview.blank?
          atom_to_take_as_preview = my_atoms.first
        else
          atom_to_take_as_preview = atom_flagged_as_preview
        end
        preview_atom = self.atoms.select{ |atom| atom.name == atom_to_take_as_preview["name"] }.first
        unless preview_atom.blank?
          ...
          elsif preview_atom.atom_type == "Atoms::Picture"
...
private
  ...
  def self.all_for_layout(page, page_layout = "standard")
    ...
    molecule_names = Alchemy::PageLayout.molecule_names_for(page_layout)
    ...
    molecules_already_on_the_page = page.molecules
  ...
  # makes a copy of source and makes copies of the atoms from source
  def self.copy(source, differences = {})
    differences[:position] = nil
    attributes = source.attributes.except("id").merge(differences)
    molecule = self.create!(attributes.merge(:create_atoms_after_create => false, :id => nil))
    source.atoms.each do |atom|
      new_atom = Atom.copy(atom, :molecule_id => molecule.id)
      new_atom.move_to_bottom
    end
    molecule
  end
  
  # creates the atoms for this molecule as described in the molecules.yml
  def create_atoms
    molecule_scratch = my_description
    atoms = []
    if molecule_scratch["atoms"].blank?
      logger.warn "\n++++++\nWARNING! Could not find any atom descriptions for molecule: #{self.name}\n++++++++\n"
    else
      molecule_scratch["atoms"].each do |atom_hash|
        atoms << Atom.create_from_scratch(self, atom_hash.symbolize_keys, {:created_from_molecule => true})
      end
    end
  end
