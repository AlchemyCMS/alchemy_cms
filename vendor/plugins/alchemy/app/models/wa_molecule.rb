class WaMolecule < ActiveRecord::Base
  require 'yaml'
  ...
  # Returns next WaMolecule on self.wa_page or nil. Pass a WaMolecule.name to get next of this kind.
  def next(name = nil)
    if name.nil?
  ...
  # Returns previous WaMolecule on self.wa_page or nil. Pass a WaMolecule.name to get previous of this kind.
  def prev(name = nil)
    if name.nil?
      find_conditions = ["public = 1 AND wa_page_id = ? AND position < ?", self.wa_page.id, self.position]
    else
  ...
  def self.list_molecules_by_layout(layout = "standard")
    molecules = WaMolecule.descriptions
    result = []
  ...
  def my_description
    WaMolecule.descriptions.detect{ |d| d["name"] == self.name }
  end
...
private
  ...
  def self.all_for_layout(wa_page, page_layout = "standard")
    molecule_descriptions = WaMolecule.descriptions
