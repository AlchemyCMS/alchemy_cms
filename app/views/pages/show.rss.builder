...
xml.rss :version => "2.0" do
  xml.channel do
    ...
    if multi_language?
      xml.link show_page_with_language_url(:urlname => @page.urlname, :lang => session[:language])
    else
    ...
    for element in @page.elements.find_all_by_name('news')
      xml.item do
        ...
        if multi_language?
          xml.link show_page_with_language_url(:urlname => element.page.urlname, :anchor => element_dom_id(element), :lang => session[:language])
          xml.guid show_page_with_language_url(:urlname => element.page.urlname, :anchor => element_dom_id(element), :lang => session[:language])
        else
