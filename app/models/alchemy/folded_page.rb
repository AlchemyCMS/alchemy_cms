...
module Alchemy
  class FoldedPage < BaseRecord
    ...
    belongs_to :user, inverse_of: :folded_pages, class_name: Alchemy.config.auth.user_class_name

    def self.folded_for_user(user)
      return none unless Alchemy.config.auth.user_class.respond_to?(:where)
