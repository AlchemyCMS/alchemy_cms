...
describe Alchemy::Admin::DashboardController do
  ...
  context "in admin backend" do
    let(:dummy_user) { mock_model(Alchemy.config.auth.user_class, alchemy_roles: %w[admin], language: "de") }
    ...
    context "if no other parameter is given" do
      ...
      context "if user has no preferred locale" do
        let(:dummy_user) { mock_model(Alchemy.config.auth.user_class, alchemy_roles: %w[admin], language: nil) }
        ...
        context "if locale is available" do
          ...
          context "if user language is an instance of a model" do
            ...
            let(:dummy_user) { mock_model(Alchemy.config.auth.user_class, alchemy_roles: %w[admin], language: language) }
