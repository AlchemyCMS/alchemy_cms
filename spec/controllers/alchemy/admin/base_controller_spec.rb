...
describe Alchemy::Admin::BaseController do
  ...
  describe "#permission_denied" do
    context "when called with an AccessDenied exception" do
      ...
      it "redirects to config.auth.login_path if no user" do
        controller.send(:permission_denied, CanCan::AccessDenied.new)
        expect(controller).to have_received(:redirect_to).with(Alchemy.config.auth.login_path)
      end

      it "redirects to config.auth.unauthorized_path for a logged in user" do
        authorize_user(build(:alchemy_dummy_user))
        controller.send(:permission_denied, CanCan::AccessDenied.new)
        expect(controller).to have_received(:redirect_to).with(Alchemy.config.auth.unauthorized_path)
      end
