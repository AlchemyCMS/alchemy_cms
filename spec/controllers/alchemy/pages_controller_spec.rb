  ...
  describe PagesController do
    ...
    describe "#index" do
      ...
      describe "url nesting" do
        ...
        before do
          allow(Alchemy.config.auth.user_class).to receive(:admins).and_return(OpenStruct.new(count: 1))
      ...
      describe "#page_etag" do
        ...
        context "with user logged in" do
          before do
            authorize_user(mock_model(Alchemy.config.auth.user_class, cache_key_with_version: "bbb"))
          end

          it "returns another etag for response headers" do
            expect(subject).to include(an_instance_of(Alchemy.config.auth.user_class))
          end
        end
      end
    end
