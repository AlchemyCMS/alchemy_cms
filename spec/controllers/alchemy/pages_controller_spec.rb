...
describe Alchemy::PagesController do
  ...
  describe "#index" do
    ...
    describe "url nesting" do
      ...
      before do
        allow(Alchemy.user_class).to receive(:admins).and_return(double(count: 1))
    ...
    describe "while redirecting" do
      context "not in multi language mode" do
        before do
          allow_any_instance_of(described_class).to receive(:multi_language?).and_return(false)
        end
        ...
        context "with no lang parameter present" do
          it "should store defaults language id in the session." do
            ...
            expect(controller.session[:alchemy_language_id]).to eq(Alchemy::Language.default.id)
          end

          it "should store default language as class var." do
            get :show, params: { urlname: page.urlname }
            expect(Alchemy::Language.current).to eq(Alchemy::Language.default)
          end
        end
      end
    end
