  ...
  describe Page do
    ...
    describe "#readable_by?" do
      ...
      context "for an unrestricted page" do
        let(:page) { build(:alchemy_page, restricted: false, restricted_roles: []) }

        it { is_expected.to be(true) }
      end

      context "for a restricted page the user has a role for" do
        let(:page) { build(:alchemy_page, restricted: true, restricted_roles: %w[member]) }

        it { is_expected.to be(true) }
      end

      context "for a restricted page the user has no role for" do
        let(:page) { build(:alchemy_page, restricted: true, restricted_roles: %w[restricted_test]) }

        it { is_expected.to be(false) }
      end

      context "for a restricted page without any role" do
        let(:page) { build(:alchemy_page, restricted: true, restricted_roles: []) }
