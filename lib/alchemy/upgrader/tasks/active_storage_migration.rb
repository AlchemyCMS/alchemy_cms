        ...
        class << self
          def migrate_picture(picture)
            Alchemy::Deprecation.silence do
              ...
              copy_file(Rails.root.join("uploads/pictures", uid), key)
            end
          end
          ...
          def migrate_attachment(attachment)
            Alchemy::Deprecation.silence do
              ...
              copy_file(Rails.root.join("uploads/attachments", uid), key)
            end
          end
          ...
          def copy_file(uid, key)
            ...
            when S3_SERVICE
              if S3::Client.exist?(uid)
                service.send(:make_path_for, key)
                S3::Client.cp uid, service.send(:path_for, key)
              end
            end
          end
