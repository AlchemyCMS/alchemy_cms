    ...
    class << self
      ...
      def read_file(file)
        ...
        if Gem::Version.new(Psych::VERSION) >= Gem::Version.new('3.1.0.pre1')
          ::YAML.safe_load(File.read(file), permitted_classes: YAML_WHITELIST_CLASSES, permitted_symbols: [], aliases: true)
        else
          ::YAML.safe_load(File.read(file), YAML_WHITELIST_CLASSES, [], true)
        end
      rescue Errno::ENOENT
        {}
      end
